...
class DashboardController extends Controller
{
    public function index($other_user_id=null)
    {
        ...
        {
            $get_all_tem_members = DB::select("SELECT GROUP_CONCAT(user_pk_no) team_members FROM t_teambuild WHERE (team_lead_user_pk_no=$user_id OR hod_user_pk_no=$user_id OR hot_user_pk_no=$user_id)")[0]->team_members;
            $get_all_tem_members .= $get_all_tem_members.",".$user_id;
        }
        else
        {
        ...
        $avt_data = DB::select("SELECT user_pk_no,team_lead_user_pk_no,user_name,yy_mm,target_amount,target_by_lead_qty,sold_yymm,sold_amt FROM kpi_avt WHERE team_lead_user_pk_no=$user_id and team_lead_user_pk_no!=user_pk_no");
        $apt_data = DB::select("SELECT user_pk_no,team_lead_user_pk_no,user_name,lead2k1,k12priority,priority2sold,k12sold FROM kpi_apt WHERE team_lead_user_pk_no=$user_id and team_lead_user_pk_no!=user_pk_no");
        $acr_data = DB::select("SELECT user_pk_no,team_lead_user_pk_no,user_name,k1_count,k1_priority_ratio,priority_count,priority_sold_ratio,sold_count FROM kpi_acr WHERE team_lead_user_pk_no=$user_id and team_lead_user_pk_no!=user_pk_no");
        //PERFORMANCE STATISTICS END

        // TODO LIST START
        $next_followup = DB::select("SELECT COUNT(lf.lead_pk_no) next_followup_cnt
            FROM t_leadfollowup lf
            JOIN t_leadlifecycle llc ON (lf.lead_pk_no = llc.lead_pk_no)
            WHERE lf.Next_FollowUp_date >= CURRENT_DATE")[0];
        $missed_followup = DB::select("SELECT COUNT(lf.lead_pk_no) missed_followup_cnt
            FROM t_leadfollowup lf
            JOIN t_leadlifecycle llc ON (lf.lead_pk_no = llc.lead_pk_no)
            WHERE lf.Next_FollowUp_date < CURRENT_DATE")[0];
        // TODO LIST END

        return view('admin.dashboard',compact('lead_count','k1','priority','sold','hold','closed','transferred','accepted','avt_data','apt_data','acr_data','next_followup','missed_followup'));
    }
        ...
        if($type == "avt")
        {
            ...
            if(!empty($avt_chart_data))
            {
                foreach ($avt_chart_data as $cdata) {
                    ...
                    $chart_arr[$i]['value'] = $cdata->target_amount;
                    $i++;
                }
            }
        }
        ...
        if($type == "apt")
        {
            ...
            if(!empty($apt_chart_data))
            {
                foreach ($apt_chart_data as $cdata) {
                    $chart_arr[$i]['Lead2k1'] = 'Lead2k1';
                    $chart_arr[$i]['lead2k1_value'] = $cdata->lead2k1;
                    $chart_arr[$i]['k12priority'] ='k12priority';
                    $chart_arr[$i]['k12priority_value'] = $cdata->k12priority;

                    $chart_arr[$i]['priority2sold'] = 'priority2sold';
                    $chart_arr[$i]['priority2sold_value'] = $cdata->priority2sold;
                    $chart_arr[$i]['k12sold'] = 'k12sold';
                    $chart_arr[$i]['k12sold_value'] = $cdata->k12sold;
                    $i++;
                }
            }
        }
        ...
        if($type == "acr")
        {
            ...
            if(!empty($apt_chart_data))
            {
                foreach ($apt_chart_data as $cdata) {
                    ...
                    $chart_arr[$i]['k1_count'] = $cdata->k1_count;

                    $chart_arr[$i]['k1_priority_ratio'] ='k1_priority_ratio';
                    $chart_arr[$i]['k1_priority_ratio_value'] = $cdata->k1_priority_ratio;

                    $chart_arr[$i]['priority_count'] = 'priority_count';
                    $chart_arr[$i]['priority_count_value'] = $cdata->priority_count;

                    $chart_arr[$i]['priority_sold_ratio'] = 'priority_sold_ratio';
                    $chart_arr[$i]['priority_sold_ratio_value'] = $cdata->priority_sold_ratio;

                    $chart_arr[$i]['sold_count'] = 'sold_count';
                    $chart_arr[$i]['sold_count_value'] = $cdata->sold_count;
                    $i++;
                }
            }
        }
