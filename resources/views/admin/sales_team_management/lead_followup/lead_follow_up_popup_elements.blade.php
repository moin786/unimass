		...
		<table class="table">
			<tbody>
				...
				<tr>
					...
					<th>Category:</th>
					<td>
						{{ isset($lead_data)? $lead_data->project_category_name:'' }}
				...
				<tr>
					...
					<td>
						...
						<input type="hidden" class="keep_me" name="sales_agent_id" value="{{ isset($lead_data)? $lead_data->lead_sales_agent_pk_no:'' }}" readonly="readonly"/>
					</td>
					<th>Created by:</th>
				...
				<tr>
					...
					<td>{{ isset($lead_stage_arr[$lead_data->lead_current_stage])? $lead_stage_arr[$lead_data->lead_current_stage]: " " }}</td>
