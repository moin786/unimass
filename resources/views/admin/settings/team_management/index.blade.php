...
<script>
	$(function () {
		...
		$(document).on("change","#user_name",function(){
			...
			{
				$("#user-table tbody").html("<tr><td>"+user_name+"<input type='hidden' name='txtUserID[]' value="+user_id+" ><input type='hidden' name='teammem_id[]' value='' ></td><td align='center'><input type='checkbox' name='chkIsHod"+user_id+"[]' /></td><td align='center'><input type='checkbox' name='chkIsHot"+user_id+"[]' /></td><td align='center'><input type='checkbox' name='chkIsTL"+user_id+"[]' /></td><td><input class='form-control' style='width: 50px;' type='text' name='teammem_seq"+user_id+"' /></td><td class='text-center'><span class='btn bg-danger btn-xs remove-member' data-id='' ><i class='fa fa-close'></i></span></td></tr>");
			}
		});
